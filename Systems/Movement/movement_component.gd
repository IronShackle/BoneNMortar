...
func _physics_process(delta: float) -> void:
	...
	character_body.velocity = current_velocity + external_impulse
...
## Apply external impulse (knockback, lunge, etc)
...
func start_dodge(direction: Vector2) -> void:
	var dodge_dir = direction.normalized() if direction.length() > 0.1 else last_input_direction
	if dodge_dir.length() < 0.1:
		dodge_dir = Vector2.RIGHT
	var dodge_strength = dodge_distance / dodge_duration
	apply_impulse(dodge_dir * dodge_strength)
